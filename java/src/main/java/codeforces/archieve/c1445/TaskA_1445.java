package codeforces.archieve.c1445;
...
public class TaskA_1445 {
    ...
    static void solve() {
        ...
        List<Integer> b = FS.readListInt(n);

        a.sort(Comparator.comparingInt(it -> it));
        b.sort(Comparator.comparingInt(it -> it));
        for (int i = 0; i < n; i++) {
            if (a.get(i) + b.get(n - i - 1) > x) {
                FS.pt.println("NO");
                return;
            }
        }
