package codeforces.archieve.c1453;
...
1 4 3 2 4 1
 */
public class Task1453_B {

    static void solve() {
        int n = FS.nextInt();
        List<Long> a = FS.readListLong(n);
        long f = Math.abs(a.get(0) - a.get(1));
        long s = Math.abs(a.get(n - 1) - a.get(n - 2));
        long third = Long.MIN_VALUE;
        for (int i = 1; i < n - 1; i++) {
            third = Math.max(third, Math.abs(a.get(i) - a.get(i - 1)) + Math.abs(a.get(i + 1) - a.get(i)) - Math.abs(a.get(i + 1) - a.get(i - 1)));
        }
        long total = 0;
        for (int i = 0; i < n - 1; i++) {
            total += Math.abs(a.get(i) - a.get(i + 1));
        }
        FS.pt.println(total - Math.max(f, Math.max(s, third)));
    }
