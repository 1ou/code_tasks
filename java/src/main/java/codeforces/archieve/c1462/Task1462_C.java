package codeforces.archieve.c1462;
...
public class Task1462_C {
    ...
    static void solve() {
        ...
        int y;
        do {
            y = res.stream().mapToInt(e -> e).sum();
            if (x > y + xx) {
                res.add(xx--);
            } else {
                res.add(x - y);
            }
        } while (res.stream().mapToInt(e -> e).sum() < x && xx != 0);

        if (res.stream().mapToInt(e -> e).sum() != x) {
            FS.pt.println(-1);
            return;
        }
        res.sort(Comparator.naturalOrder());
        for (int i = 0; i < res.size(); i++) {
            FS.pt.print(res.get(i));
        }
        FS.pt.println();
    }
