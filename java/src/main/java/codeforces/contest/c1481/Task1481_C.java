...
import codeforces.archieve.c1263.A;
...
public class Task1481_C {
    ...
    static void solve() {
        ...
        for (int i = 0; i < m; i++) {
            List<Integer> malars = colors.get(c[i]);
            if (malars == null) {
                malars = new ArrayList<>();
                malars.add(i);
                colors.put(c[i], malars);
            } else {
                malars.add(i);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            if (a[i] != b[i]) {
                List<Integer> malars = colors.get(b[i]);
                if (malars != null && malars.size() > 0) {
                    ans[malars.get(malars.size() - 1)] = i;
                    malars.remove(malars.size() - 1);
                    a[i] = b[i];
                } else {
                    FS.pt.println("NO");
                    return;
        ...
        for (Map.Entry<Integer, List<Integer>> it : colors.entrySet()) {
